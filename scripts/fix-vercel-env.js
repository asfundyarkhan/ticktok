...
console.log(`${colors.cyan}=== Vercel Deployment Fix Tool ===${colors.reset}\n`);
console.log(`${colors.yellow}This script will help fix environment variable issues with Vercel deployments${colors.reset}\n`);
...
const possibleCredentialFiles = [
  ...
  path.join(process.cwd(), 'ticktokshop-5f1e9-firebase-adminsdk-fbsvc-34fc84c417.json'),
  ...
  if (fs.existsSync(file)) {
    try {
      ...
      if (fileContent.type === 'service_account') {
        ...
        console.log(`${colors.green}✓ Found Firebase credentials: ${path.basename(file)}${colors.reset}`);
        break;
      }
    } catch (error) {
...
if (!firebaseCredentialsFile) {
  console.log(`${colors.yellow}⚠️ No Firebase admin credentials file found${colors.reset}`);
}
...
const envVariables = {
  ...
  ...getExistingEnvVariables(['.env', '.env.local', '.env.development', '.env.production']),
  ...
  NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID: process.env.NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID || "YOUR_MESSAGING_SENDER_ID",
...
if (firebaseCredentialsFile) {
  const firebaseCredentials = JSON.parse(fs.readFileSync(firebaseCredentialsFile, 'utf8'));
  ...
  console.log(`${colors.green}✓ Added Firebase admin credentials to .env.local${colors.reset}`);
}
...
console.log(`${colors.yellow}1. Go to your Vercel dashboard: https://vercel.com/dashboard${colors.reset}`);
console.log(`${colors.yellow}2. Select your project${colors.reset}`);
console.log(`${colors.yellow}3. Go to Settings > Environment Variables${colors.reset}`);
console.log(`${colors.yellow}4. Add the following environment variables:${colors.reset}\n`);
...
console.log(`   ${colors.yellow}- Copy the entire key including "-----BEGIN PRIVATE KEY-----" and "-----END PRIVATE KEY-----"${colors.reset}`);
console.log(`   ${colors.yellow}- When pasting in Vercel, paste it as is, Vercel will handle the formatting${colors.reset}\n`);

console.log(`${colors.cyan}=== Alternative Quick Setup (if Vercel CLI is installed) ===${colors.reset}`);
if (hasVercelCLI) {
  console.log(`${colors.yellow}Run this command to add all environment variables at once:${colors.reset}\n`);
  console.log(`${colors.green}vercel env pull .env.production.local${colors.reset}`);
  console.log(`${colors.green}vercel env push .env.local${colors.reset}\n`);
} else {
  console.log(`${colors.yellow}Install Vercel CLI first with: npm install -g vercel${colors.reset}\n`);
}

console.log(`${colors.magenta}=== Next Steps ===${colors.reset}`);
console.log(`${colors.yellow}1. Verify your environment variables are set correctly in Vercel${colors.reset}`);
console.log(`${colors.yellow}2. Deploy again with: vercel --prod${colors.reset}\n`);

console.log(`${colors.green}Done! Your Vercel deployment should now work correctly.${colors.reset}`);
