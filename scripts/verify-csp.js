...
const requiredDomains = [
  ...
  { directive: 'connect-src', domain: '*.firebaseinstallations.googleapis.com' },
  ...
  for (const site of sitesToTest) {
    ...
    try {
      ...
      for (const required of requiredDomains) {
        ...
        if (!directiveValues.some(value => 
          value === required.domain || 
          (required.domain.startsWith('*.') && value.includes(required.domain.substring(2))))) {
