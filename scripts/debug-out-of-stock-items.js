...
async function debugStockItems() {
  try {
    ...
    console.log(
      "⚠️  Note: This script needs Firebase configuration to run against real data."
    );
    ...
    const shouldAppear = mockStockItems.filter(
      (item) => item.listed === true && typeof item.stock === "number"
    );
    ...
    console.log(
      `- Listed items: ${mockStockItems.filter((i) => i.listed).length}`
    );
    console.log(
      `- Out-of-stock items: ${
        mockStockItems.filter((i) => i.stock === 0).length
      }`
    );
    console.log(
      `- Out-of-stock AND listed: ${
        mockStockItems.filter((i) => i.stock === 0 && i.listed).length
      }`
    );

    console.log("\n🎯 Expected Behavior:");
    console.log(
      '- Out-of-stock items (stock: 0) should appear with "Out of Stock" badge'
    );
    console.log(
      '- Items should have "Add Stock" and "Edit" buttons when stock is 0'
    );
    console.log(
      "- Items should NOT be deleted from database when stock reaches 0"
    );

    console.log("\n🔧 Potential Issues to Check:");
    console.log("1. Database validation in subscribeToAdminStock method");
    console.log(
      "2. Cleanup services still running despite being commented out"
    );
...
function mockSubscribeToAdminStock() {
  ...
  mockFirestoreData.forEach((doc) => {
    console.log(
      `  ${doc.id}: stock=${doc.data.stock} (${typeof doc.data.stock})`
    );
  });
  ...
  mockFirestoreData.forEach((doc) => {
    ...
    console.log(
      `  ${doc.id}: valid=${isValid} (stock type: ${typeof data.stock})`
    );
...
debugStockItems().then(() => {
  ...
  console.log("\n🎯 Next Steps:");
  console.log(
    "1. Check database directly for any items with stock: null or undefined"
  );
  console.log(
    "2. Verify that stock updates are properly setting numeric values"
  );
  console.log(
    "3. Check if any cleanup services are running despite being disabled"
  );
