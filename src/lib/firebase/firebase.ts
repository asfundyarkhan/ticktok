...
const firebaseConfig: {
  ...
  storageBucket:
    ...
    "ticktokshop-5f1e9.firebasestorage.app", // Updated to use correct bucket format
  messagingSenderId:
...
try {
  ...
  if (!getApps().length) {
    console.warn(
      "Attempting fallback Firebase initialization with minimal config"
    );
    try {
      ...
      console.warn(
        "Fallback Firebase initialization successful with minimal config"
      );
    } catch (fallbackError) {
      console.error(
        "Fatal: Firebase initialization completely failed",
        fallbackError
      );
      ...
      if (process.env.NODE_ENV === "production") {
        try {
          ...
          console.warn(
            "Emergency Firebase initialization with incomplete configuration"
          );
        } catch (e) {
          throw new Error(
            "Could not initialize Firebase client: " +
  ...
  if (!auth) {
    try {
      ...
      if (process.env.NODE_ENV === "production") {
        console.warn(
          "Auth will not function correctly - some features will be unavailable"
        );
      } else {
        throw e;
      }
  ...
  if (!firestore) {
    try {
      ...
      if (process.env.NODE_ENV === "production") {
        console.warn(
          "Firestore will not function correctly - some features will be unavailable"
        );
      } else {
        throw e;
      }
  ...
  if (!storage) {
    try {
      ...
      if (process.env.NODE_ENV === "production") {
        console.warn(
          "Storage will not function correctly - some features will be unavailable"
        );
      } else {
        throw e;
      }
