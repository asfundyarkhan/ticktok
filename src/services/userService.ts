...
export class UserService {
  ...
  static async generateReferralCode(uid: string): Promise<string> {
    try {
      ...
      if (userData.role !== 'admin' && userData.role !== 'superadmin') {
        throw new Error('Only admin or superadmin users can generate referral codes');
      }
  ...
  static async validateReferralCode(referralCode: string): Promise<{isValid: boolean; adminUid?: string}> {
    try {
  ...
  static async upgradeToSellerWithReferral(uid: string, referralCode: string): Promise<boolean> {
    try {
  ...
  static async getUsersReferredByAdmin(adminUid: string): Promise<UserProfile[]> {
    try {
      const q = query(
        collection(firestore, this.COLLECTION),
  ...
  static async getAllReferralRelationships(): Promise<{ user: UserProfile; referrer: UserProfile }[]> {
    try {
      ...
      const querySnapshot = await getDocs(q);
      const referralPromises: Promise<{ user: UserProfile; referrer: UserProfile | null }>[] = [];
      ...
      querySnapshot.forEach((doc) => {
        ...
        if (referredBy) {
          ...
          const referralPromise = this.getUserById(referredBy).then(referrer => {
            return {
              user: userProfile,
              referrer: referrer
            };
      ...
      return relationships.filter(rel => rel.referrer !== null) as { user: UserProfile; referrer: UserProfile }[];
    } catch (error) {
  ...
  static async getAllReferralCodes(): Promise<{uid: string; email: string; role: string; referralCode: string}[]> {
    try {
      const q = query(
        collection(firestore, this.COLLECTION),
      ...
      const querySnapshot = await getDocs(q);
      const referralCodes: {uid: string; email: string; role: string; referralCode: string}[] = [];
