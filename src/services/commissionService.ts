...
export class CommissionService {
  private static readonly COMMISSION_BALANCES_COLLECTION = "commission_balances";
  private static readonly COMMISSION_TRANSACTIONS_COLLECTION = "commission_transactions";
  ...
  static async recordSuperadminDeposit(
    ...
    try {
      const commissionAmount = depositAmount * this.COMMISSION_RATE;      return await runTransaction(firestore, async (transaction) => {
        ...
        const balanceRef = doc(firestore, this.COMMISSION_BALANCES_COLLECTION, adminId);
        ...
        const commissionTransactionRef = doc(collection(firestore, this.COMMISSION_TRANSACTIONS_COLLECTION));
   ...
   */
  static async getAdminCommissionSummary(adminId: string): Promise<CommissionSummary> {
    try {
      ...
      const totalCommissionBalance = totalFromSuperadminDeposits + totalFromReceiptApprovals;
  ...
  static async getTotalCommissionBalance(): Promise<{
    ...
    try {
      const balancesQuery = query(collection(firestore, this.COMMISSION_BALANCES_COLLECTION));
      ...
      const transactionsQuery = query(collection(firestore, this.COMMISSION_TRANSACTIONS_COLLECTION));
      const transactionsSnap = await getDocs(transactionsQuery);
