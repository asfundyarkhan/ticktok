...
export interface ActivityItem {
  ...
  type: 'seller_account_created' | 'stock_purchased' | 'fund_deposit' | 'fund_withdrawal' | 'product_sold';
  details: {
    quantity?: number;
    productName?: string;
    amount?: number;
...
export class ActivityService {
   ...
   */
  static async createActivity(activity: Omit<ActivityItem, 'id' | 'createdAt'>): Promise<string> {
    try {
      ...
      return onSnapshot(
        ...
        (error) => {
          ...
          if (error.code === 'failed-precondition') {
            console.error('Missing index for activities query. Create a composite index on:');
  ...
  static formatActivityMessage(activity: ActivityItem): string {
    const time = activity.createdAt.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' });
    switch (activity.type) {
      case 'seller_account_created':
        return `[${time}] ${activity.userDisplayName} created a seller account${
          activity.details.referralCode ? ` via referral code ${activity.details.referralCode}` : ''
        }`;
