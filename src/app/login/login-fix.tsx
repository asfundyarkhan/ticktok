...
function LoginForm() {
  ...
  // Track whether a redirect is in progress to prevent loops
  ...
  const handleLogin = async (values: LoginFormValues) => {
    ...
    try {
      ...
      if (user && !user.emailVerified) {
        ...
        // Set that we're starting a redirect to prevent loops
        setRedirectInProgress(true);
        
        // Use a longer timeout to ensure Firebase auth state is complete
        setTimeout(() => {
      ...
      // Add delay before redirection to ensure authentication is complete
      setTimeout(() => {
        ...
        if (redirectParam) {
          ...
          console.log(`Redirecting to provided path: ${redirectParam}`);
      ...
      if (error instanceof FirebaseError) {
        ...
        switch (error.code) {
          ...
          case "auth/email-not-verified":
            ...
            // Redirect to verify email page after a longer delay to prevent reload issues
            setTimeout(() => {
              window.location.href = "/verify-email";
            }, 3000);
            break;
      ...
      // We don't set isLoading to false here if we're redirecting
