...
if (!admin.apps.length) {
  try {
    ...
    admin.initializeApp({
      ...
      databaseURL: process.env.FIREBASE_DATABASE_URL || "https://ticktok-7b7b6-default-rtdb.firebaseio.com"
    });
...
async function debugProductIds() {
  try {
    ...
    console.log(`Found ${listingsSnapshot.size} items in sellerListings collection\n`);
    ...
    if (stockSnapshot.size > 0) {
      ...
      if (firstData.productCode) {
        try {
          const byCodeQuery = await db.collection('adminStock')
