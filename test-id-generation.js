...
import { generateAdminProductId, generateProductCode, generateUniqueId, generateUUID } from '../src/utils/idGenerator';
...
function testUniqueIdGeneration() {
  ...
  console.log(`✅ Generated ${stressTestIds.size} unique IDs in ${endTime - startTime}ms\n`);
...
function oldProblematicIdGeneration() {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    if (oldIds.has(id)) {
      ...
      console.log(`   Previous occurrences: ${Array.from(oldIds).filter(oid => oid === id).length}`);
      return;
    }
    oldIds.add(id);
  }
